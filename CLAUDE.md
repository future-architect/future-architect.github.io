...
- **コードフェンスの言語は「中身」で選ぶ**（#2505）。中身と噛み合わないと強調表示が丸ごと消える
  - **プロンプト（`$`）付きの端末の画面は `console`**。highlight.js が `$ ` をプロンプトとして切り出し、
    コマンド部分だけを Bash として着色して出力行はそのまま残す
  - **スクリプト・単体のコマンドは `sh` か `bash`**。どちらでもよい（同じ Bash として描画される）
  - **実行結果・ログだけなら `text`**。`bash` にすると出力がシェルの構文として着色される
  - **`shell` と `terminal` は使わない。** `shell` は名前がスクリプトを想像させるが実体は
    `console` の別名で、実際に158件中78%が中身と食い違っていた。`terminal` は highlight.js に
    登録が無く、プレーン表示になる
  - `❯` `➜` `%` `>` `dbname=#` のようなプロンプトは highlight.js が認識しない。
    コマンドと出力が混ざるので `text` にする（`$` に書き換えてまで `console` にはしない）
  - 再発は textlint の `no-mismatched-code-fence` が止める
