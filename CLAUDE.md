...
- 実効サイズの基準:
  ...
  | 本文見出し h2〜h6 | `1.85 / 1.5 / 1.3 / 1.2 / 1.1em` = 24.05 / 19.5 / 16.9 / 15.6 / 14.3px（太さは全て700。直前の空きは h2 56px / h3 32px / h4 以下 24px）。`text-wrap` は指定しない（#2822。`balance` が実際に効くのは幅375pxの列だけで〈PC で2行になる見出しは 0.9%〉、そこで1行目が列の65%しか埋めず折り返しが早く見えた） | 〃 |
...
- **パンくずの現在地は太さで示す**（#2705 / #2722）。`_partial/breadcrumb.ejs` は
  ...
  記事ページとカテゴリ一覧は同じ3項目（`ホーム > カテゴリ > Programming`）が並ぶので、
  **末尾がリンクかどうかしか違いが無く、読者には同じ表示に見えていた**
