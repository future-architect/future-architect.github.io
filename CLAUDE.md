...
- `.textlintrc`: `preset-ja-technical-writing` + `spellcheck-tech-word`。一文200文字まで、漢字連続10文字まで。感嘆符・疑問符と弱い表現は許容
  ...
  - **`ja-no-mixed-period` は「漢字・かな終わり」だけを見る**（#2381）。`allowPeriodMarks` に
    ...
    - **コード・表・画像・引用・箇条書きの直前の段落は対象外**（`.textlint/textlint-filter-rule-block-label`）。
      この位置の段落はブロックのラベル・導入であって文ではなく、句点を求めても
      「▼ サンプルコード。」にしかならない。長さでは切っていない（何文字までがラベルかを
      決められないため、位置で決める）
...
- 実効サイズの基準:
  ...
  | 記事タイトル | `clamp(28px, 1.325rem + 0.9vw, 32px)` | 〃（1箇所のみ） |
...
- **見出しの階層はサイズ・直前の空き・罫線の3つで作る**（#2524）。
