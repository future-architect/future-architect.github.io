...
**色だけでリンクを名乗りません。** 色が見えない読者には、青い文字も紫の文字も隣の黒い文字とほぼ同じ濃さに見えます。白背景で AA を保ったまま本文の文字と 3:1 を作れる色は存在しないので、リンクであることは**下線**が示し、色はそれに添えます。下線が付くのは本文・箇条書き・表のセルの中のリンクで、見出しのアンカーやカードの中のリンクには付きません。
