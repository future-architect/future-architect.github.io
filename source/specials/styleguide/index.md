...
**`ink-faint` は明るいほうと同じく「4.5 を満たす最も薄い段階」です。** いちばん厳しい背景（`surface-mute`）で 4.74 で、これより薄くしません。

**ブランドのネイビーは、暗い背景では 1.05 しか出ません。** 面に使っても線に使っても消えるので、フッターの帯は1段階暗い値にして色味だけ残し、選択中のページ・選択中のタブの上辺・共有ボタン・タグの hover・フォーカスリングは明るい線と明るい塗りに置き換えています。
...
**背景だけ明るいまま残すと、中の文字がテーマ側に付いていって沈みます。** note の4色と表彰の淡い金は、背景ごと暗くしています。ページの背景との差は 1.25〜1.40 で、面のスケール（`surface-mute` の 1.21）より1段階上です。
