...
`**` が効くのは、**合計の行や、特に見てほしい1行・1セル**だけを囲んだときです。
...
[`git status`](https://example.com/) のように、リンクでインラインコードを包むこともできます。
```
...
[^1]: 脚注の中身です。`gofmt -l .` のようなインラインコードや[リンク](https://example.com/)も書けます。記事の一番下にまとめて出ます。
```
