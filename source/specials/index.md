---
title: 特設ページ
description: "フューチャー技術ブログの特設ページの一覧です。エンジニア組織の紹介、ガイドラインや教材（リソース）、コンテスト・ポッドキャスト・アドベントカレンダー（アクティビティ）、記法ガイドとスタイルガイドへの入口をまとめています。"
layout: page
---

# 特設ページ

フューチャー技術ブログの特設ページの一覧です。

## フューチャーについて

このブログを書いている組織の説明です。

- [フューチャーのエンジニア組織](/specials/engineers/) — 技術戦略と、現場のITコンサルタントが技術の進め方で大事にしていること

## リソース

フューチャーが社外へ公開している知識資産です。

- [フューチャーのガイドライン](/specials/guidelines/) — 設計ガイドライン・コーディング規約・TypeScript 教材の入口

## アクティビティ

技術を通じた社外向けの活動です。

- [HACK TO THE FUTURE](/specials/httf/) — フューチャー主催のプログラミングコンテストと、競技プログラミング記事の入口
- [アドベントカレンダー](/specials/advent-calendar/) — 2015年から毎年参加している Qiita Advent Calendar の歴代一覧
- [Future Tech Cast](/specials/techcast/) — 技術ブログから派生したポッドキャストの番組紹介とエピソード一覧
- [出版・寄稿](/specials/publications/) — 社員が書いた書籍と、技術誌への寄稿の記録

## このブログについて

このブログの書き方と見た目の決まりです。

- [記法ガイド](/specials/markdown/) — 記事に使える記法と、その表示結果の見本
- [スタイルガイド](/specials/styleguide/) — 色・文字・形の決まりと、その実物
- [部品ギャラリー](/specials/gallery/) — サイトを組み立てている部品を、実物のまま並べたもの
