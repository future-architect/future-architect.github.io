...
- 利点1 `time.Sleep` を含むテストの実行時間を圧縮できる
  - バブル内では time パッケージはフェイククロックを用います。バブル内の全 goroutine が durably blocked になった段階でバブル内の時間が前進し、 `time.Sleep` は実時間を待たずに完了します。この性質を利用することで、 `time.Sleep` を含むテストコードの実行時間を圧縮できます
...
::: note info `cancel` と `AfterFunc` の goroutine 関連の挙動について補足
...
`synctest.Test` + `synctest.Wait` を使うことで、 **「実行時間が伸びること」と「flaky であること」という 2 つの課題を同時に解決できる**、ということが分かります。
