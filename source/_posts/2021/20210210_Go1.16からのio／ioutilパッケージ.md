...
`io/ioutil` パッケージは主にパッケージのインポートサイクルを回避するために存在します。[Codebase Refactoring](https://talks.golang.org/2016/refactor.article) にあるようにGoの `io` パッケージは `os` パッケージに依存できません。`io/ioutil` パッケージが直接/間接的に `os` パッケージと `io` パッケージを参照することで、インポートサイクルを回避したということです。

<img src="/images/2021/20210210/package_dependency.svg" class="img-middle-size" width="333" height="329" loading="lazy">
