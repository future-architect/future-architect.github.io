...
各モジュールについて、クラス定義を解析していきます。`NodeModule` が指定するパスを読み込み、`visit_ClassDef` を実装した `ClassDefNodeVisitor` で処理します。また、`ClassDefNodeVisitor` には親ノードへのポインタを追加の情報として持たせています。`generic_visit` の前後で親ノードを入れ替えることで、クラス内クラスなども表現できます。
