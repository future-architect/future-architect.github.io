...
Flutterは[公式サイト](https://flutter.dev/?gclid=Cj0KCQiAnuGNBhCPARIsACbnLzrdf197SlPULqzczv4M2p5mNGCENYpEki6GWdiN1kdeKzzJ_KishTcaAuDLEALw_wcB&gclsrc=aw.ds)からSDKをインストールすることで`flutter`コマンドが利用できるようになります。`flutter`コマンドはアプリケーションの初期構築、起動、依存モジュールの管理ができます、`npm`コマンド+任意のフレームワークのCLIコマンドのような存在ですね。
...
<img src="/images/2021/20211221a/image.png" alt="サンプルアプリ画像" width="400" height="444">
...
Reactなど近年のフロントエンド フレームワークでは、画面の状態を管理することで、状態が更新された場合コンポーネント単位で再描画する、更新のないコンポーネントは再描画しない、という必要最小限の画面更新をする仕組みでアプリケーションのパフォーマンスが高くなるように設計されています。Flutterもその仕組みを採用しており、状態管理をしているということですね。FlutterではStateを持たない`StatelessWidget`、State管理の仕組みを備えた`StatefulWidget`の２種類のウィジェットが存在します。
