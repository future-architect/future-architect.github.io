...
ただし、与えられた最大値・最小値からどのようにサイズを決定するかは、ウィジェットによって異なり、またウィジェットの親子関係によっても変わってきます。各ウィジェットのサイズ決定については私自身まだ理解できていない部分が多くありますが、一例としてContainer ウィジェットのサイズの挙動について紹介します。Container ウィジェットは明示的にサイズを指定することも可能ですが、今回は直接的な指定は行わない場合のサイズの挙動・調整について記載します。

### Container ウィジェットのサイズ

Container ウィジェットのサイズは、子要素の有無で違ってきます。
...
### Column ウィジェットでラップした際のContainer ウィジェットのサイズ

単純にColumn ウィジェットでラップした場合は、最小限のサイズになります。

さらにContainer ウィジェットをExpanded ウィジェットでラップすると、メイン軸方向(Columnの場合は垂直方向）にサイズを拡張してくれます。また、Expanded ウィジェットの`Flex`プロパティで、各ウィジェットを均等な大きさにしたり大きさの割合を指定できます。
