...
author: 伊藤真彦
lede: "Dart/Flutter入門に参加します、DartといえばFlutterの話が必ずついてくるものですが、今回は連載1記事目として、敢えてプログラミング言語としてのDartに焦点を絞った記事にします。"
---
...
<img src="/images/2021/20210510b/logo_lockup_dart_horizontal.png" alt="dart horizontal logo" width="560" height="202" loading="lazy">
...
しかし、2018年にDart2として再起動、モバイルアプリケーション向けフレームワークである`Flutter`の基本ライブラリでDartが採用される事により、近年注目度が上昇しています。今iOS/Androidのクロスプラットフォームでアプリケーションを開発するならDartが熱い、という事ですね。

## 技術的特徴

Dartはクラスベースのオブジェクト指向言語です、単一継承のみがサポートされていますが、Mixinも利用できます。
...
JavaScriptトランスパイラにより、作成したコードをJavascriptに変換できます。デバッグビルドでのみ動作する`assert`という構文があるのも特徴です。
...
どのような型でも再代入可能になる一方で、`dynamic obj = 1;`をそのまま`"1" + obj`でString型の文字列と結合できません。
...
例えばコンパイル段階で計算できない実行時の時刻を`const`で宣言できません。
...
`String`型の変数は、シングルクオートまたはダブルクオートで文字列を作成できます。
...
インスタンス変数、コンストラクタ、メソッドを指定して、クラスを定義できます。Personクラスの定義の内部にPerson()を定義する形でコンストラクタを設定します。
