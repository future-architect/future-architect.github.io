...
まず次のようなInterfaceと、このInterfaceを満たすStructのSQLを実行する共通コードを用意しておきます。
...
本番データに起因するバグが発生した際などに、検証環境で安全かつスムーズに調査するために生まれたものです。

本番DBのスナップショットをリストア、データマスキングを行った後再度スナップショット化。
RDSのスナップショットは指定したアカウントにシェアできるので、これを検証環境にリストアすることで本番データを検証環境にクローンしています。
