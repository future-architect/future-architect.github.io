...
画面上部の GitHub アイコンの隣にある白黒の🕓️アイコンをクリックすると、履歴パネルが開きます。`Save Current State` を押すと現在の編集状態がスナップショットとして残り、あとで一覧から選んで戻せます。

大きな編集をするときは、`Save Current State` でスナップショットを残すか、Live Editor を別タブで開いてそちらで試すのがお勧めです。元タブの URL（`#pako:...`）にその時点のコードが保持されているので、別タブの試行が失敗しても元タブに戻ればすぐやり直せます。`Save Current State` の保存先はブラウザの `localStorage` なので、別PC・別ブラウザには引き継がれない点に注意してください。
