...
<img src="/images/20250827a/a94c1d88-020c-4467-bb98-70b0e151f4ad.png" alt="" width="709" height="575" loading="lazy">
...
以下は、上記各種フレームワーク等で出現する式について一部抜粋したものです。ここに関しては、また別記事でご紹介したいと思います。

- $リスク = 資産 × 脅威 × 脆弱性$
こちらは情報セキュリティの分野で、リスクの構造を分析するために用いられる基本式です。特定の規格で数式として厳密に定義されているというよりは、リスクの因果関係を説明するためのフレームワークとして認知されています。
- $リスク = 影響度 × 発生確率$
こちらはリスク評価における最も古典的かつ一般的な定義で、リスクマネジメントの基本概念として広く定着しています。国際的な標準規格でも採用されています。
cf. ISO 31000 (リスクマネジメント), NIST SP 800-30, PMBOK (プロジェクトマネジメント知識体系ガイド)
- $リスク（ALE）= 資産価値（AV）× 暴露係数（EF）× 年間発生率（ARO）$
こちらはリスク対策を経営層に説明する際など、ビジネス上の意思決定で特に強力なのが、リスクを年間損失額として算出する指標、年間損失予測 (ALE)です。 リスクを「金額」で評価します。
SLE (Single Loss Expectancy)：$1回あたりの損失額 = 資産価値 (AV) × 曝露係数 (EF)$
