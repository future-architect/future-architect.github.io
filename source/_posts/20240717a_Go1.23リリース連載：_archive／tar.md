---
title: "Go1.23リリース連載: archive/tar"
...
上記が実装されていれば、`uid/gid`→`/uname/gname` のルックアップを行わせずに済むような制御を加えることができます。`tar`コマンドの `--numeric-owner`オプション相当の挙動にする場合はともかく、やりたいことによっては引数に `uid/gid` が必要じゃないか？って思いましたが、`fs.FileInfo` からフィールドにアクセスできるようで、シンプルなシグネチャです。
...
コードを書き換えます。` FileInfoNames`インターフェースを満たす `emptyFileInfoNames`を宣言して利用します。
