...
`describe`がグループを表し、`describe 名詞`で、特定の要素のテストであるという宣言をし、その中で`it 振る舞い`というのを並べていきます。この`it`は英語的には`describe`の名詞を指します。特定の要素に対して、その振る舞いを記述するというスタイルです。
...
<img src="/images/2023/20230825a/スクリーンショット_2023-07-18_15.09.32.png" alt="" width="251" height="125">
...
`--grep-invert`で含んでいないものを実行したり、`|`でORを表現したり正規表現でANDを表現もできます。
