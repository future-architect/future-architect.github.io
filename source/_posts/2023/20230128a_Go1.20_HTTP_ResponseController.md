...
<img src="/images/2023/20230128a/top.png" alt="" width="800" height="457" loading="lazy">
...
インターフェースを拡張したいときに `ResponseWriter` インターフェースとは別のインターフェースにAPIを定義して、`ResponseWriter` インターフェースを実装する型に別インターフェースのメソッドを実装させる、というのが1つの手段としてあります。一例はHTTPハンドラがバッファリングしているデータをクライアントにフラッシュできる `http.Flusher` インターフェースです。
...
のようなインターフェースの定義もできたでしょう。しかし `ResponseWriter` 型をラップしたミドルウェアとの相性など、使い勝手が悪いため見送られています。
