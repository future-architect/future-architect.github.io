...
categories:
  - Programming
series: "Go1.21リリース"
thumbnail: /images/2023/20230816a/thumbnail.png
author: 宮永崇史
lede: "Go1.21に追加されたslicesパッケージの解説を行います。今回注目するslicesパッケージは、Go1.21以前にはx/exp/slicesとして運用されていたものが正式に取り込まれ、多数の関数が追加され.."
---

<img src="/images/2023/20230816a/eyecatch-20230814.png" alt="" width="1200" height="675" loading="lazy">
...
さて、記事のメインテーマとは異なる事柄を挟みましたが、ここからはGo1.21に追加されたslicesパッケージを解説します。
...
- コメント
  ...
  - ↑を調べたところ、[comment#45038](https://github.com/golang/go/issues/45038#issuecomment-819685966)に以下のコメントがされていました
...
- 説明
  ...
  - 最初に一致しない要素が見つかった場合、要素を比較し、s1の要素<s2の要素の場合は-1を返却します(その逆で1を返却します)
...
- コメント
  - MaxFuncを使えば構造体のスライスに対してフィールドの最大値の評価もできます
...
- コメント
  - MinFuncを使えば構造体のスライスに対してフィールドのさいしょ値の評価もできます
