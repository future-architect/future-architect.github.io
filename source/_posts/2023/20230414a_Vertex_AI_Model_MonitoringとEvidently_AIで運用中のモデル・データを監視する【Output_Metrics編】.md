...
[前回の記事](/articles/20230413a/)の`今回想定するユースケース`の部分と合わせてあるので、詳細は割愛しますが、以下の前提条件で検証します。
...
<img src="/images/2023/20230414a/2023-04-03-09-41-52.png" alt="" width="1200" height="464">
...
* 実装コスト
  * 評価すること自体はVertex AIのコンソール画面から設定できるのでほぼ実装コストは掛かりません
...
* [★2] 説明変数列（特徴量データの列）, 正解データ列(`target`), 予測結果データ列(`prediction`) を含むデータ
  ...
  * 予測結果データ列はVertex AIで手動でバッチ予測し、その結果を新たに`prediction`列として追加する
...
* 過去・現在の2つのデータで評価して結果を比較したい場合は[★3]と[★4]ように別のCSVファイルを指定する
...
※Evidently AIのTestについては[前回の記事](/articles/20230413a/)に詳細があります。
...
本記事ではVertex AIとEvidently AIを用いたOutput Metrics監視を検証しました。\
