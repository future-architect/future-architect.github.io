...
作成したデータセット（2011年の時間毎シェアバイク利用者数のデータ）で学習します。\
...
モデルは完成しましたが、そのモデルを使って予測する窓口（エンドポイント）を作成する必要があります。\
...
Evidently AIでの監視はVertex AI Model Monitoringのようにノーコードで実現できませんが、\
...
また、今回はHTMLファイル（or Notebookのセル出力）でTestの結果を表示しましたが、結果をJSONファイルとして出力もできます。
...
Evidently AIは、Vertex AI Model Monitoringのようにノーコードで実現できません。簡素ではありますが、Pythonコードを書く**実装コスト**、及び定期的にモニタリングを実行する仕組み（**運用コスト**）が必要です。
