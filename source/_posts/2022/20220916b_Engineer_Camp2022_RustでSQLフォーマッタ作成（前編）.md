...
1. 厳密な構文解析せずにトークンベースで処理を行っているため、難しい処理がある（例: 括弧の処理、エイリアスの補完など）
1. Pythonで書かれているためVSCodeの拡張機能で動かすことが困難である

そこで、2020年のインターンシップにおいて、構文解析を利用したTypeScript製フォーマッタを作成しました([記事](/articles/20200919/))。
しかし、このフォーマッタは実行速度が非常に遅く、実用的ではありませんでした。遅くなっていた理由は、このフォーマッタはANTLRを用いて構文解析しており、ANTLRのSQLパーサとTypeScriptの相性が悪く、実行速度が遅かったためでした。
...
今回作成したフォーマッタの処理の流れを以下に示します。フォーマッタは、構文解析部分とフォーマット処理部分の2つに分かれています。まず、対象となるソースファイルに構文解析し、構文木(CST)を構築します。そして、構文木に基づいてフォーマット処理を行い、SQLを整形しています。

<img src="/images/2022/20220916b/stream.png" alt="stream.png" width="960" height="228" loading="lazy">

構文解析する部分は上述した[tree-sitter-sql](https://github.com/m-novikov/tree-sitter-sql)を使用し、フォーマット処理を行う部分は0から実装しました。
実装の詳細は後編で述べます。
...
今回、私たちはRust言語を用いてSQLフォーマッタを作成しました。現状では実用的なレベルとは言えませんが、実際に動作するフォーマッタを開発できたことは貴重な経験でした。今後は実用的なレベルのフォーマッタを改良していきたいと考えています。
