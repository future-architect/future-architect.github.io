...
---
<img src="/images/2022/20220606b/23855886_s.jpg" alt="" width="640" height="427">
...
* 報酬
  * 時給1500円/H。1日あたり12,000円（※8時間勤務した場合）
  * 実際の勤務時間に基づいて支給いたします
* 交通費
  * 規定に沿って来社時の交通費を支給します。
...
Engineer Campではバラエティ豊かな14コースを用意しています。それぞれに特徴があり、使う技術も様々です。すべてのコースに共通するスキル要件は3つありますが、非常にベーシックな内容です。
...
続けてコースごとの詳細です。 `応募条件(あれば歓迎）` を見れば、大体どのような技術で開発するかが分かると思います。
...
RustでSQLフォーマッタを作るためRust製パーサ・ジェネレータ/パーサの調査・検証、あるいはパーサを自作していただきます※インターンシップ参加者のレベルに合わせて内容レベルを調整する想定です。
...
2. **新規サービスで未検討のアイデアを深堀り、プロト作成**
    * 新規サービスのアイデアの深掘りと必要な機能を提案したのち、実際に開発まで行っていただきます。
...
大手プロパンガス供給会社の配送員の仕事の効率化に取り組みます。現行システムで課題となっていた交換頻度や変動の予測精度を改善するのが我々のミッションです。新たなIoTデバイスを用いたビッグデータを解析することにより、最適な配送計画を立案できるようなDX化にチャレンジしてもらいます。ビッグデータ解析やIoTに興味のある方におすすめです。
