...
前提知識となる、 `splice(2)` ですが、入力用と出力用のファイル記述子を繋ぎ、カーネル空間とユーザー空間でのデータコピーをせず（ゼロコピーと言われる所以です）、データ転送するシステムコールです。`(2)` の `2` は引数ではなく、システムコールを指す番号です。
