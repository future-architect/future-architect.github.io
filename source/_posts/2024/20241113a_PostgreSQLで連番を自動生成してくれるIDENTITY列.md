...
自動的に連番が登録される便利機能ですが、実は明示的に値を登録できてしまいます。
...
さて、IDENTITY列にはオプションがありまして、 `BY DEFAULT` の代わりに `ALWAYS` が指定できます。これを利用すると、IDENTITY列に明示的に値を指定できなくなります。
...
余談ですが、該当カラムに明示的にインサートしていることを示しつつ、値が自動採番を用いることを示したい場合は `DEFAULT` キーワードを利用できます。
