---
title: "リリース直前にライブラリのインストールエラーが発生した際にどのように対応したか - Glue Python Shell起動エラーの対応"
...
---
<img src="/images/2024/20240119a/d6788854-dfac-4797-979e-3f40531a1eee.jpeg" alt="" width="1024" height="1024" loading="lazy">
...
そのため、 `awswrangler` を明示的にインストールせずとも利用できるため、
...
ただ、どちらにせよリスクは発生するため、単体テストと AWS 環境上での結合テストを再度実行し、動作保証の上で本番稼働しました。
...
ちなみに、AWS Glue の追加ライブラリにおいて、lock ファイル利用したライブラリのバージョンを固定する方法は現時点ではわかりませんでした。
