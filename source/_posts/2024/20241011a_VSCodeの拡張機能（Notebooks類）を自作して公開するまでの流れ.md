...
VSCode は、多くの IT 従事者に愛用されています。開発、コンパイル、テストに限らず、コードレビューや障害調査、ドキュメント整理など、さまざまな場面で活躍します。さらに、拡張機能を活用すれば、日常の繰り返し作業を自動化もできます。
...
- Input
  ...
  - また、テストでダミーデータが必要な場合も多いので、数年前に作った [json-creator](https://www.npmjs.com/package/json-creator) を使って、テスト用の JSON 形式のダミーデータを生成も可能です。
- Extract
...
親の開発画面（F5 を押す側）で Breakpoint を入れてデバグしたりもできます。ソース上の `console.log()`のメッセージも開発画面側の DEBUG CONSOLE ウィンドウで確認できます。
...
作成した拡張機能を VSIX 形式のにパックすれば、他人に共有できます。
