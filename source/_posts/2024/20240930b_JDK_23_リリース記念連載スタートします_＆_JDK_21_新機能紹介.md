...
---
<img src="/images/2024/20240930b/4119073_s.jpg" alt="4119073_s.jpg" width="640" height="427">
...
レコードパターンは、Javaのパターンマッチング機能を拡張するものです。具体的には、レコードクラスの内部にあるコンポーネントを直接抽出し、それらを変数にバインドできます。これにより、複雑なデータ構造を簡潔に処理することが可能になります。
...
レコードパターンはネストもできますので、レコードの中にレコードがある場合でも一度に必要な情報を抽出できます。
...
レコード `Rectangle` については構成要素の `ColoerdPoint` に分解できますが、構成要素がレコードの場合さらに分解できます。
...
`Thread.Builder` インターフェースを使用すると、今まで `new Thread()` で作成していたような一般的なスレッドを作成できます。`Thread.Builder.OfPlatform` によってプラットフォーム・スレッドが作成され、`Thread.Builder.OfVirtual` によって仮想スレッドが作成されます。
