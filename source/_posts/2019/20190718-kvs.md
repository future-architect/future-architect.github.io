...
また、keyに対して複数のカラムを定義でき、カラム型にはListやMapのようなコレクション型、いわゆるオブジェクト型のようなユーザ定義型（UDT:User Defined Type）の使用に対応しているため、JSONのような階層的なデータをスキーマ定義して柔軟に扱うことができるのが特徴です。
...
CassandraではRDBと同様にデータをテーブルという単位で管理します。
KVSではデータの管理、アクセスはkeyに対して行うため、PRIMARY KEYとして定義する必要があります。
Valueに相当するカラムは複数定義可能で配列やMapなどのコレクション型もデータ型として定義できます。
...
また、配列やMapで定義した項目に対しても要素の追加や削除など柔軟に更新できます。

#### Map型のデータ更新

先ほど作成したtest_tableのMap型で定義したkeywordに対して要素を追加・更新してみましょう。
...
<img src="/images/2019/20190718/photo_20190718_01.png" class="img-middle-size" width="454" height="147" loading="lazy">
...
UDTのフィールドにMapなどのコレクション型も定義できます。
...
Cassandraのデータモデルを考える際にはついついRDB的な思考で結合ありきのデータモデルでテーブルを設計してしまいますが、まずはテーブルを集約できるかを念頭に見つめ直してみましょう。
