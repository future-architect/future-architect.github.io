...
<img src="/images/2019/20191113/go-cdk-logo-gopherblue.png" class="img-middle-size" width="900" height="489" loading="lazy">
...
Query()の返り値に対して、fluentインターフェースで情報を付与し、最後にGet()を呼び出します。次のメソッドがあります。どれもSQLでおなじみですね。
...
複数のオペレーションをまとめて実行するのがアクションリストです。ほとんどのストレージAPIはバルクでまとめて処理を渡すことで効率よく処理できたりしますが、このアクションリストはそれを活用するためのAPIになっています。

Actions()で帰ってきた`docstore.ActionList`に対してfluentインターフェースでメソッドを複数呼び、最後にDoを呼び出すことでまとめて実行されます。
