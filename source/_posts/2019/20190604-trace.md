...
各処理にどれだけ時間がかかったのかを、ルールに従って出力することにより、複雑に関連しあったタスクでどこに問題があるのかを一目で見られるようにする。たんなるログではなく、親子関係の構造を持つログ。
...
* スパンに対してログ、タグといったものの出力もできる
* 分散トレースの文脈では、スパンの集合=トレース

この情報を使うことで、下記のようなグラフを表示できる。

<img width="993" height="628" alt="" src="/images/2019/20190604/opencentus_dashboard.png">
...
1. ローカル開発では、分散トレース収集可視化のJaeger/Zipkinと、メトリクス収集可視化のPrometheusがDockerで簡単に起動できるので、これを出力先にして開発する
