...
わざわざ設定しなくても、編集したいJSONファイルの中からスキーマを指定できます。この記法はVSCodeでもIntelliJでも有効でした。スキーマは相対パスでローカルのスキーマファイルを指定できますし、URLを書くこともできそうです（試した時にはファイルがプライベートリポジトリだったので失敗しましたが）。
...
Goから扱うのも簡単です。JSON to Goというウェブサイトを僕はよく使いますが、これを使うと、生のJSONから、それを解釈するための構造体を作って来られます。これで、encoding/jsonを使った読み書きがだいぶ簡単になります。これ以外にもいろいろあります。[JSON Schema や JSON から Go の struct を生成する](https://qiita.com/aaharu/items/ed578fc0e3e42dbcc4b4)というエントリーにまとまっています。
...
<img src="/images/2019/20191001/1.png" width="1000" height="573" loading="lazy">
...
さて、上記の絵だと、どこから作成しても、他のものを生成できて、自由に作り始められそうな感じはあります。しかし、やはり作りやすい・作りにくいという順序はあります。よくあるちょっとした自動化でもありがちですが、ワークフローを考える上で考慮すべきことはいくつかあります。
