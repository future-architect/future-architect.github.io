...
基本方針の中には、修正方法を開発者に伝えないようなエラーメッセージはなくしていくとか、「トークン」のようなコンパイラ内部用語（ジャーゴン）を外に出さないようにする、という話があります。コンパイラ視点ではなく、プログラマー視点の用語の「文字」を出すようにしていくというのは面白いなと思いました。単にTypeScriptとかに変換処理を投げるだけじゃなくて、その出力もラップして、開発者にとって使いやすい処理系を目指しています。なかなかに野心的なプロジェクトです。
