...
Go[インターフェース](https://yourbasic.org/golang/interfaces-explained/)はJavaインタフェースに似ていますが、Goインタフェースの場合、インタフェースが要求するメソッド群を提供している型はみな、そのインタフェースの実装として扱われます。明示的な宣言は必要ありません。
