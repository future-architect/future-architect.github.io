...
ElasticsearchはElastic社からオープンソースで公開されており、誰でもインストールして無料で利用できます。
...
もし変更したい場合は `config/elasticsearch.yml` の `#http.port: 9200` のコメントアウトを外して別のポートも指定できます（再度起動すると反映されます）。
...
検索時には、クエリ文字列に対しても形態素解析しその結果（今回は**「春」** **「祭り」**）を条件に検索をかけ、Hitしたドキュメントが返却されるという仕組みです（索引型検索）
...
本記事ではドキュメントの登録や検索などのほんの（本当に！）一部しか紹介していませんが、より高度な条件の検索をかけたり、登録ドキュメントを分析する様々な種類があります。
