...
GraphQLはWeb API用のクエリ言語です。誤解を恐れずに書けばWebAPIにおけるSQLのような存在です。GraphQL（つまりクエリ）と呼ぶとSelectしかない印象を持ってしまいそうですが、Mutationと呼ばれるスキーマを定義すればInsert/Update/Delteが相当の処理を実行できます。今回はMutationは省略してQueryのみを実装していきます。
