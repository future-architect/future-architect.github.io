...
async function ensureKroki() {
  ...
  throw new Error(`kroki が ${KROKI_URL} で起動しない。docker compose -f ${COMPOSE_FILE} logs を確認`);
}
...
async function render(hash, entry) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        throw Object.assign(new Error(`HTTP ${res.status}: ${body.slice(0, 300)}`), {fatal: res.status < 500});
      }
  ...
  throw new Error(`${entry.files.join(', ')} の図 (${hash.slice(0, 12)}) の生成に失敗: ${lastError.message}`);
}
...
async function mapLimit(items, limit, fn) {
  ...
  await Promise.all(Array.from({length: Math.min(limit, items.length)}, async () => {
    while (index < items.length) {
      const i = index++;
      results[i] = await fn(items[i]).then((value) => ({value}), (error) => ({error}));
    }
  }));
  return results;
}
...
console.log(`図 ${diagrams.size} 件 / 生成済み ${diagrams.size - missing.length} 件 / 未生成 ${missing.length} 件 / 不要 ${orphans.length} 件`);
