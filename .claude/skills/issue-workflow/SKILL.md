...
- **その PR で issue の残作業がゼロになるなら、本文に `Closes #<n>` を書く。**
  タイトルの `(#<n>)` は参照だけで閉じない。複数 PR で進める issue（#2381 の
  ルール取捨、#2605 の年単位の修正など）には書かない。部分対応で閉じてしまう。
  自動クローズはマージ時点で走り、デプロイ完了前であることも留意する
- 閉じるときは issue にもコメントを残す。**範囲外にしたもの・残した判断**は、
  PR 本文ではなく issue 側に無いと後から辿れない
